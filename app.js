...
function editVolunteer(id) {
    ...
    if (volunteer) {
        ...
        document.getElementById('annee_integration').value = volunteer.annee_integration || '';
...
function renderVolunteers(filteredVolunteers = null) {
    ...
    tbody.innerHTML = displayList.map((volunteer, index) => `
        <tr>
            ...
            <td>${getNombreAnnees(volunteer.annee_integration)}</td>
...
function exportToCSV() {
    ...
    const headers = [
        ...
        'Email', 'Adresse', 'Zone', 'Année intégration', 'Compétences', 'Formations', 'Disponibilité', 
...
// ==========================================
    ...
    function exportToPDF() {
        ...
        doc.setFontSize(20);
        doc.text('Comité Local de Tabarre', 105, 15, { align: 'center' });
        doc.setFontSize(12);
        doc.text('Croix-Rouge Haïtienne', 105, 22, { align: 'center' });
        doc.setFontSize(16);
        doc.text('Registre des Volontaires', 105, 28, { align: 'center' });

        // Table data
        const tableData = volunteers.map(v => [
            `${v.nom} ${v.prenom}`,
            v.sexe === 'M' ? 'Masculin' : 'Féminin',
            v.date_naissance ? new Date(v.date_naissance).toLocaleDateString('fr-FR') : '-',
            v.telephone,
            v.zone,
            getNombreAnnees(v.annee_integration),
            getFormationDisplayNames(v.formations),
            v.disponibilite,
            v.statut
        ]);
        ...
        doc.autoTable({
            startY: 40,
            head: [['Nom Complet', 'Sexe', 'Naissance', 'Téléphone', 'Zone', 'Années', 'Formations', 'Disponibilité', 'Statut']],
            body: tableData,
            theme: 'grid',
        ...
        doc.text(`Total: ${volunteers.length} volontaires - Généré le ${new Date().toLocaleDateString('fr-FR')}`, 105, finalY, { align: 'center' });

        // Download
        doc.save(`volontaires_cltcrh_${getDateString()}.pdf`);
...
function getNombreAnnees(anneeInt) {
    if (!anneeInt) return '-';
    const currentYear = new Date().getFullYear();
    const annee = parseInt(anneeInt);
    if (isNaN(annee) || annee > currentYear) return '-';
    return (currentYear - annee) + ' ans';
}
